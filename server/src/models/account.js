...
export class AccountDAO {
  ...
  /** 查找账号（仅 id 和密码）— 登录用 */
  findForLogin(phone) {
    return this.pool.query('SELECT id, password FROM accounts WHERE phone = ?', [phone]);
  }

  /** 检查邮箱是否已被其他账号绑定 */
  isEmailUsed(email, excludePhone) {
    if (excludePhone) {
      return this.pool.query('SELECT id FROM accounts WHERE notify_email = ? AND phone != ?', [email, excludePhone]);
    }
    return this.pool.query('SELECT id FROM accounts WHERE notify_email = ?', [email]);
  }

  /** 根据 phone 获取 notify_email */
  getNotifyEmail(phone) {
    return this.pool.query('SELECT notify_email FROM accounts WHERE phone = ?', [phone]);
  }

  /** 获取 DeepSeek API Key */
  getApiKey(phone) {
    return this.pool.query('SELECT deepseek_api_key FROM accounts WHERE phone = ?', [phone]);
  }
  ...
  updateFields(phone, updates, params) {
    return this.pool.query(`UPDATE accounts SET ${updates.join(', ')} WHERE phone = ?`, [...params, phone]);
  }
