...
export async function getCourseList(instance) {
  try {
    ...
    await instance.rateLimiter.acquire({ random: { min: 500, max: 3000 } });
    await instance.axios.get('https://mooc2-ans.chaoxing.com/mooc2-ans/visit/interaction', {
      headers: cfg.headers
    }).catch(() => {});

    await instance.rateLimiter.acquire({ random: { min: 500, max: 3000 } });
    const resp2 = await instance.axios.post(
      'https://mooc2-ans.chaoxing.com/mooc2-ans/visit/courselistdata',
      new URLSearchParams({ courseType: '1', courseFolderId: '0', query: '', superstarClass: '0' }).toString(),
      { headers: { ...cfg.headers, 'Content-Type': 'application/x-www-form-urlencoded' } }
    );
...
export async function getCoursePoint(instance, courseId, clazzId, cpi) {
  try {
    const url = `https://mooc2-ans.chaoxing.com/mooc2-ans/mycourse/studentcourse?courseid=${courseId}&clazzid=${clazzId}&cpi=${cpi}&ut=s`;

    await instance.rateLimiter.acquire({ random: { min: 500, max: 3000 } });
    const resp = await instance.axios.get(url, { headers: cfg.headers });
...
export async function getJobList(instance, course, point) {
  const allJobs = [];
  let jobInfo = null;
  const maxRetries = 6;

  instance._currentKnowledgeId = point.id;
  ...
  for (let num = 0; num < maxRetries; num++) {
    ...
    try {
      await instance.rateLimiter.acquire({ random: { min: 500, max: 3000 } });
      const resp = await instance.axios.get(url, { headers: cfg.videoHeaders });
      const html = typeof resp.data === 'string' ? resp.data : '';
