...
export async function getVideoStatus(instance, objectId) {
  const fid = instance.getFid();
  ...
  for (let retry = 0; retry < 3; retry++) {
    ...
    try {
      await instance.rateLimiter.acquire({ random: { min: 0, max: 2000 } });
      const resp = await instance.axios.get(url, { headers: cfg.videoHeaders, timeout: 15000 });
...
export async function videoProgressLog(instance, course, job, jobInfo, dtoken, duration, playingTime, type = 'Video', isdrag = 3) {
  const userid = await instance.getUid();
  ...
  for (const rt of rtValues) {
    ...
    try {
      await instance.rateLimiter.acquire({ random: { min: 0, max: 2000 } });
      const resp = await instance.axios.get(baseUrl, {
        params,
...
export async function refreshVideoStatus(instance, job, type = 'Video') {
  return await getVideoStatus(instance, job.objectid);
}

export async function recoverAfterForbidden(instance, job, type = 'Video') {
  SessionManager.updateCookies();

  let status = await refreshVideoStatus(instance, job, type);
  if (status) return status;

  if (instance.account.password) {
    logger.info('重新登录...');
    const loginResult = await instance.login(false);
    if (loginResult.status) {
      SessionManager.updateCookies();
      status = await refreshVideoStatus(instance, job, type);
    }
  }
